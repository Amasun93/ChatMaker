"""One semantic contract for governed LLMWiki indexes, pages, and packs."""
...
class LLMWikiSemanticError(Exception):
...
def _yaml_mapping(raw: bytes, *, reason: str, path: str | None = None) -> dict[str, Any]:
    if not isinstance(raw, bytes):
        raise LLMWikiSemanticError(reason, path=path)
    try:
        text = raw.decode("utf-8")
        value = yaml.load(text, Loader=_UniqueKeyLoader)
    except (UnicodeDecodeError, yaml.YAMLError) as exc:
        raise LLMWikiSemanticError(reason, path=path) from exc
...
def validate_index_bytes(
    ...
    *,
    ...
    """Validate the exact compact-index identity and frozen eight-section map."""

    value = _yaml_mapping(raw, reason="llmwiki_index_invalid", path="llmwiki/index.yaml")
    if (
    ...
    for section in sections:
        ...
        if (
            ...
            or any(
                not isinstance(topic, str) or _SAFE_ID.fullmatch(topic) is None
                for topic in topics
            )
            or section.get("pack_id") != mapped_pack_id
            or (
                expected_pack_id is not None
                and section.get("pack_id") != expected_pack_id
            )
        ):
            raise LLMWikiSemanticError(
                "llmwiki_index_section_invalid", path="llmwiki/index.yaml"
...
def _split_page(raw: bytes, *, path: str) -> tuple[bytes, bytes]:
    if not isinstance(raw, bytes):
        raise LLMWikiSemanticError("llmwiki_page_utf8_invalid", path=path)
    try:
        raw.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise LLMWikiSemanticError("llmwiki_page_utf8_invalid", path=path) from exc
    if raw.startswith(b"---\r\n"):
        opening = len(b"---\r\n")
        closing = b"\r\n---\r\n"
        ...
        raise LLMWikiSemanticError("llmwiki_page_frontmatter_invalid", path=path)
...
def validate_page_bytes(
    ...
    *,
    ...
    page_path = path or f"llmwiki/sections/{expected_section_id}.md"
    frontmatter_raw, body_raw = _split_page(raw, path=page_path)
    if not body_raw.strip() or len(body_raw) > MAX_BODY_BYTES:
        raise LLMWikiSemanticError("llmwiki_page_body_size_invalid", path=page_path)
    if expected_board_id not in BOARD_IDS or expected_section_id not in SECTION_IDS:
        raise LLMWikiSemanticError("llmwiki_page_identity_invalid", path=page_path)
    frontmatter = _yaml_mapping(
        frontmatter_raw,
        reason="llmwiki_page_frontmatter_invalid",
        path=page_path,
    )
    source_refs = (
        list(expected_source_refs)
        if expected_source_refs is not None
        else [SOURCE_REFS[expected_board_id]]
    )
    expected = {
        "schema_version": "1.0",
        "kind": "llmwiki-page",
    ...
    if set(frontmatter) != _PAGE_KEYS or frontmatter != expected:
        raise LLMWikiSemanticError("llmwiki_page_frontmatter_invalid", path=page_path)
...
def validate_pack_payload(
    ...
    *,
    ...
    """Validate one complete official pack payload from any storage boundary."""
    ...
    if set(files) != expected_paths:
        raise LLMWikiSemanticError("llmwiki_page_set_mismatch")
    index = validate_index_bytes(
        files["llmwiki/index.yaml"],
        expected_board_id=expected_board_id,
        expected_pack_id=expected_pack_id,
    )
    for section_id in SECTION_IDS:
        page_path = PurePosixPath("llmwiki", "sections", f"{section_id}.md").as_posix()
        validate_page_bytes(
            files[page_path],
            expected_board_id=expected_board_id,
