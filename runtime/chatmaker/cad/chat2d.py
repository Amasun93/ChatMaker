...
const s=clone(initial);s.items=s.board?[clone(s.board)]:[];let selected=s.items.length?0:null,rightTab="library",librarySearch="",libraryKind="all",drag=null,rx=-25,ry=32;
...
function templateSvg(item){const pad=Math.max(4,Math.min(item.width,item.depth)*.18),w=item.width+2*pad,d=item.depth+2*pad;return'<svg viewBox="'+(-w/2)+' '+(-d/2)+' '+w+' '+d+'" aria-hidden="true">'+templateShapes(item,"")+'<text class="module-label" x="0" y="'+(item.depth/2+pad*.7)+'" text-anchor="middle">'+esc(item.revision||item.name)+'</text></svg>'}
...
function addCustomTemplate(){const name=prompt("自定义模板名称","");if(name===null||!name.trim())return;const width=measuredNumber("实测宽度（mm）",1,600),depth=width===null?null:measuredNumber("实测深度（mm）",1,600);if(width===null||depth===null)return;const item={id:"custom-"+Date.now(),kind:"custom",category:"自定义",name:name.trim(),revision:"实测模板",width,depth,holes:[],measurement_required:true,note:"自定义模板：未知孔和开口不会自动生成。",visual:{outline_shape:"rectangle",holes:[],center_marks:[],features:[],edge_badges:[]}};s.library.push(item);libraryKind="all";librarySearch="";renderRight();message("模板已加入图库，请把它拖到木板上")}
...
function libraryFields(){const cards=s.library.filter(item=>(libraryKind==="all"||item.kind===libraryKind)&&(item.name+" "+item.revision).toLowerCase().includes(librarySearch.toLowerCase())).map(item=>{const warning=item.measurement_required?((item.visual.center_marks||[]).length?"橙色十字为孔中心，孔径仍待实测":"实体尺寸需先确认"):"";return'<button class="card" data-library="'+item.id+'"><span class="thumb">'+templateSvg(item)+'</span><strong>'+esc(item.name)+'</strong><small>'+item.width+' × '+item.depth+' mm</small>'+(warning?'<div class="warn">'+warning+'</div>':"")+'</button>'}).join("");return'<h2>可视化元件图库</h2><div class="drop-help">按住缩略图，直接拖到任意木板。绿色表示可放置，红色表示会自动吸附。</div><div class="library-tools"><input id="librarySearch" class="search" placeholder="搜索 UNO、Nano、OLED…" value="'+esc(librarySearch)+'"><select id="libraryKind"><option value="all">全部</option><option value="board" '+(libraryKind==="board"?"selected":"")+'>主控板</option><option value="component" '+(libraryKind==="component"?"selected":"")+'>星核模块</option></select></div><div class="gallery">'+(cards||'<div class="empty">没有找到匹配元件</div>')+'</div><h2>没有这个模块？</h2><button class="button" id="addCustom">＋ 新增实测模板</button>'}
