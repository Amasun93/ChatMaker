...
Treat the installed bundle as incomplete only when an internal specialist is missing. Pure generation—idea development, HTML/CSS/JavaScript, wiring and code drafts, and MakerLab-ready OpenSCAD—must work from the Skill files alone. It does not require WorkBuddy MCP registration, a host scan, or any fixed list of host tools.

Local execution is an optional second layer. When the host can run local commands, use the `chatmaker-*` CLIs documented below. WorkBuddy MCP names may be used as an optional structured adapter when they are already available, but their absence never blocks pure generation and never triggers `chatmaker-install auto`. If neither a CLI nor an optional adapter is available, report only that the requested local check, compile, upload, serial session, or render is unavailable; do not downgrade the installed Skill itself.
...
3. For a connected but unknown MCU board, run `chatmaker-board-identify --request-json '{"action":"identify","allow_temporary_firmware":true}'` through the host's local-command capability. If WorkBuddy already exposes `board_identify`, it may call that optional adapter instead; both routes reach the same local implementation. It first performs safe reads. When needed, a 临时识别程序 may run only after a 完整备份 is verified, and the original firmware must be restored and verified before identification can complete. If evidence remains ambiguous, tell the beginner where the model/version is normally printed; if they still cannot identify it, request clear 正反面照片 and help visually. Do not expose USB IDs, bus addresses, registers, or raw commands unless diagnosis actually needs them.
4. After the exact board identity is known, read the matching `start-here` section before choosing the next specialist path. Run `chatmaker-knowledge --request-json '{"action":"section","board_id":"<exact-board-id>","consumer":"chatmaker","section_id":"start-here"}'`; an already available WorkBuddy `knowledge_get` adapter is equivalent but optional. A standalone basic mechanical CAD request skips board knowledge. The current UNIHIKER M10 alpha has a canonical board record but no signed Knowledge pack; route it to ChatDuino's `unihiker-m10.md` reference and project checker instead of inventing a `knowledge_get` result.
