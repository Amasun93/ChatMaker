...
- On the ChatMaker preview route, call `cad_openscad_status` before generation. If OpenSCAD is ready, continue without reinstalling it.
- If it is missing, explain in one short sentence and ask for explicit permission to install. The request to model, the confirmed task card, and “开始生成” are not installation permission. Only after the user says to install may you call `cad_openscad_prepare` with `allow_install=true`.
