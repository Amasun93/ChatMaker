...
4. When the project uses an owned Starcore module, read that exact component ID with the `component-profile` action. The complete handoff provides 22 companion component profiles. Use only source-reviewed dimensions; if a button cap, potentiometer shaft, limit-switch hole, or other feature says `requires_measurement`, keep the outline but ask for a real measurement instead of inventing a hole or cutout.
...
10. For a placement-aware enclosure, pass either `parameters.component_ids` for a deterministic initial layout or the shared `parameters.placements` exported by Chat2D. Chat3D accepts top and bottom placements, rejects overlap or out-of-bounds layouts, creates base standoffs and lid mounting holes from callable profiles, and cuts only source-reviewed round/rectangular panel features. A component whose mounting geometry is `requires_measurement` remains visible but gets no invented hole. When the user has no MakerWorld account, cannot log in conveniently, or explicitly prefers ChatMaker, first follow the local OpenSCAD dependency gate below, then run the `generate` action with `generation_confirmed=true` and `delivery_mode=chatmaker-preview`. Deliver both the returned `scad_code` and `preview_lab`; do not make the preview a substitute for the code. Use OpenSCAD to render the placement-aware SCAD into the final STL; the browser's built-in STL is only a basic shell preview. A nameplate preview keeps its initial wording as baked glyph geometry, so size, position, plate and hole parameters are adjustable there, while changing the characters requires regeneration through ChatCAD. Do not create screenshots or start a separate render unless requested.
