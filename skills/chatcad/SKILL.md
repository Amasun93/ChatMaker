...
4. When the project uses an owned Starcore module, read that exact component ID with the `component-profile` action. Use only source-reviewed dimensions; if a button cap, potentiometer shaft, or other feature says `requires_measurement`, ask for a real measurement instead of inventing a cutout.
...
10. When the user has no MakerWorld account, cannot log in conveniently, or explicitly prefers ChatMaker, first follow the local OpenSCAD dependency gate below, then run the `generate` action with `generation_confirmed=true` and `delivery_mode=chatmaker-preview`. Deliver both the returned `scad_code` and `preview_lab`; do not make the preview a substitute for the code. The laboratory adjusts the supported parameters and exports updated OpenSCAD plus an STL for the current parameters. A nameplate preview keeps its initial wording as baked glyph geometry, so size, position, plate and hole parameters are adjustable there, while changing the characters requires regeneration through ChatCAD. Do not create screenshots or start a separate render unless requested.
