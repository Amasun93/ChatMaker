...
8. When the user wants a name, label or any Chinese text on the lid, pass `parameters.engrave_text` (plus optional `text_size`, `text_depth`). Chinese is rendered as glyph-outline `polygon()` geometry with `linear_extrude()`, never OpenSCAD `text()`, because OpenSCAD 2021.01 and the Bambu Studio Custom 3D Print lab render CJK as tofu boxes regardless of the font file. The same outlines are triangulated directly into the STL, so no OpenSCAD version or desktop font setup is required. Rare "connected-stroke" glyphs where strokes fuse with a frame (e.g. 圈 in some fonts) may lose interior strokes; prefer regular-structure characters for labels.
