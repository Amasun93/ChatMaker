...
星核板有 8 个物理 I2C 插口，共享同一组 SCL/SDA 总线。插 OLED、NFC 等真正的 I2C 模块时：

1. 先看模块额定电压，再选插口上标有对应 `3V3` 或 `5V` 的位置。
2. 四芯线整条插入一个插口；`SCL/C` 与 `SDA/D` 是相邻信号线，不能一根插在左边、另一根插在很远的插口。
3. 代码层面它们仍对应 `P19/SCL`、`P20/SDA`，但学生不需要手工拆线寻找这两个 GPIO。

IDMS-0009 超声波是例外：它是 GPIO 的 2+2 四芯线（红 VCC、黑 GND、蓝 TRIG、绿 ECHO），推荐 `H/P26` 与 `O/P27`，不是 I2C，不能插到 SCL/SDA 插口。
