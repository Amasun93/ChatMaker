...
1. When a connected MCU board is not already exact, use the local `chatmaker-board-identify` CLI. An already available WorkBuddy `board_identify` adapter is equivalent but optional. A 临时识别程序 is allowed only after a 完整备份 is verified; always restore and verify the original program afterward. If electronic evidence still overlaps, tell the user where to look for the model/version and then request 正反面照片 if they cannot tell. Keep USB IDs, I2C addresses and registers out of the beginner-facing explanation.
2. Read the matching records under `packs/boards`, `packs/components`, and `packs/recipes` through `chatmaker-catalog` with `search` and `get` requests. WorkBuddy may use its already available `catalog_search` and `catalog_get` adapters, but MCP registration is not a prerequisite. For an onboard-hardware question, search the board name together with the hardware term (for example, `星核板 蜂鸣器`), keep a matching board result ahead of generic external modules, and continue to its Knowledge index. Treat unverified records as leads, not facts.
3. After the exact board identity is confirmed, read `identify-and-safety`, `pins-and-electrical`, and `toolchains-and-upload` for a board with a ChatMaker Knowledge index. Run `chatmaker-knowledge --request-json '{"action":"section","board_id":"<exact-board-id>","consumer":"chatduino","section_id":"identify-and-safety"}'` and substitute the requested section; an already available WorkBuddy `knowledge_get` adapter reaches the same runtime. Keep those pages paired with canonical facts rather than replacing them. The current M10 alpha instead uses [unihiker-m10.md](references/unihiker-m10.md) plus canonical board `unihiker-m10`; do not invent an unavailable Knowledge pack.
...
- Prefer a usable Mind+ 2.x installation and compile with `mindplus:esp32:mpython:FlashMode=dio,FlashFreq=80,UploadSpeed=1500000,DebugLevel=none`.
- Only when no usable Mind+ 2.x toolchain exists may the adapter fall back to Mind+ 1.x and `dfrobot:mpython:mpython:FlashMode=dio,FlashFreq=80,UploadSpeed=1500000,DebugLevel=none`. Never ask a student who already has usable Mind+ 2.x to download 1.8.
