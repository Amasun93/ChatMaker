...
- Use `chatmaker-starcore --request-json '<request>'` with the `doctor`, `ports`, `compile`, or `compile-upload` action.
...
- Reuse any usable Mind+ 1.8.x or 2.x installation already present. When both are usable, prefer Mind+ 2.x and compile with `mindplus:esp32:mpython:FlashMode=dio,FlashFreq=80,UploadSpeed=1500000,DebugLevel=none`.
- When only Mind+ 1.8.x is usable, use `dfrobot:mpython:mpython:FlashMode=dio,FlashFreq=80,UploadSpeed=1500000,DebugLevel=none`. When neither is usable, recommend the verified Mind+ 1.8.x download until the independent Starcore toolchain is available; never ask a student with a usable installation to download the other version.
