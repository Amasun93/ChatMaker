...
星核板板载 CH9102F 支持 USB 串口和自动下载控制，但一个 CH9102F 端口只能作为身份线索，不能单独证明连接的就是星核板。2026-08-24 已在一块用户确认的 v4.2.2 实板上验证唯一有线端口、编译、上传、115200 串口、完整 16 MiB 备份和恢复；板载 QMI8658 返回约 1 g 的静止数据。按键按下、实际可听蜂鸣、CAN、外接模块和断电重启仍保持 `unverified`。调用上传前仍要运行环境检查，并确认唯一合格的有线端口和正确板卡；编译成功不能替代这些验证。
