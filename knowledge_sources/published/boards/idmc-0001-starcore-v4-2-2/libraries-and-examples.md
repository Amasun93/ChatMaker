...
上面的最小程序已使用当前 Mind+ 1.8 星核板目标真实编译通过。2026-08-24 又在一块用户确认的星核板 v4.2.2 上完成上传和串口实测：8 组静止数据的合加速度为 1041–1050 mg，并在测试后恢复、验证原 16 MiB Flash。可复现实例位于 `examples/chatduino/starcore/onboard-self-test/`。这只验证当前板卡的静止加速度读取；安装方向、倾斜阈值和体感游戏仍要按实际握持方式校准。
...
该程序已在当前 Mind+ 1.8 目标编译通过。2026-08-24 实板自检读取到 A/B 均未按下，并执行了短促蜂鸣器命令；由于没有人工按键动作或现场听感确认，按下事件和实际可听蜂鸣仍保持 `unverified`。P5/P11 同时是启动相关脚；上电或复位时一直按住按键可能改变启动状态。
...
这七个示例已在当前 Mind+ 1.8 目标下真实编译通过（7/7）。当前实体板只验证了板载自检示例的烧录、串口、QMI8658 与恢复；七个外接模块没有接入，实物效果仍为 `unverified`。
