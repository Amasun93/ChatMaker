...
compile smoke tests have passed. The deterministic archive, checksum, and clean
extraction test are complete; the GitHub rc2 tag and prerelease assets remain
pending. Host UI discovery still requires restarting Codex and WorkBuddy;
physical Nano gates remain open until a wired board is available.
